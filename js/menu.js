...
let runMenu = function() {
    ...
    drawText("Fighter Player" ,MENU_ROW1, 220);
    drawText("Ranged Player" ,MENU_ROW2, 220);
        
        //Draw menu options
    drawText("Ability:" ,MENU_ROW0, menuColumnPos[0]);
    drawText(classListP1[classIndexP1] ,MENU_ROW1, menuColumnPos[0]);
    drawText(classListP2[classIndexP2] ,MENU_ROW2, menuColumnPos[0]);
    drawText("Primary:" ,MENU_ROW0, menuColumnPos[1]);
    drawText(rightHandListP1[rightHandIndexP1] ,MENU_ROW1, menuColumnPos[1]);
    drawText(rightHandListP2[rightHandIndexP2] ,MENU_ROW2, menuColumnPos[1]);
    drawText("Secondary:" ,MENU_ROW0, menuColumnPos[2]);
    drawText(leftHandListP1[leftHandIndexP1] ,MENU_ROW1, menuColumnPos[2]);
    drawText(leftHandListP2[leftHandIndexP2] ,MENU_ROW2, menuColumnPos[2]);
    drawText("Controls:" ,MENU_ROW0, menuColumnPos[3]);
        if(classIndexP1 != CLASS_P1_PALADIN){
            drawText(controlledByP1[controlIndexP1] ,MENU_ROW1, menuColumnPos[3]);
        } else {
            drawText(controlledByP1Pali[controlIndexP1] ,MENU_ROW1, menuColumnPos[3]);
        }
    drawText(controlledByP2[controlIndexP2] ,MENU_ROW2, menuColumnPos[3]);
        
        //Display previous score only if both players have died
    if(previousFighterKillCount > 0 || previousRangedKillCount > 0){
            drawText("Last Score:" ,MENU_ROW0, menuColumnPos[4]);
            drawText(previousFighterKillCount, MENU_ROW1, menuColumnPos[4]);
            drawText(previousRangedKillCount, MENU_ROW2, menuColumnPos[4]);
        }
        
        //Draw cursor
    drawImage(cursorPic,MENU_ROW1 -20 ,menuColumnPos[cursor1] - wobble);
    drawImage(cursorPic,MENU_ROW2 -20 ,menuColumnPos[cursor2] - wobble);
}
